/*
 * FBSynthcontrolsdialog.cpp - definition of FBSynthControlsDialog class.
 *
...
void FBSynthInstrumentView::mouseDoubleClickEvent(QMouseEvent *event)
{
	m_originalHeight = parentWidget()->height() == 233 ? m_originalHeight : parentWidget()->height() ;
	parentWidget()->setFixedHeight(parentWidget()->height() == m_originalHeight ? 233 : m_originalHeight );
	update();
}
