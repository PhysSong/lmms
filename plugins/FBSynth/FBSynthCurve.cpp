/*
 * FBSynthCurve.cpp - declaration of FBSynthCurve and FBSynthHandle classes.
 *
...
FBSynthHandle::FBSynthHandle(int num, int x, int y):
	m_numb(num),
	m_width(x),
	m_heigth(y),
...
QRectF FBSynthHandle::boundingRect() const
{
...
float FBSynthHandle::freqToXPixel(float freq, int w)
{
	float min = log10f(20);
...
float FBSynthHandle::xPixelToFreq(float x, int w)
{
	float min = log10f(20);
	float max = log10f(20000);
	float range = max - min;
...
float FBSynthHandle::gainToYPixel(float gain, int h, float pixelPerUnitHeight)
{
...
float FBSynthHandle::yPixelToGain(float y, int h, float pixelPerUnitHeight)
{
...
void FBSynthHandle::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	...
	if (isMouseHover())
	{
		...
		float rectX = -40;
		float rectY = -40;
		if (FBSynthHandle::y() < 40)
		{
			rectY = rectY + 40 - FBSynthHandle::y();
		}
		if (FBSynthHandle::x() < 40)
		{
			rectX = rectX + 40 - FBSynthHandle::x();
		}
		if (FBSynthHandle::x() > m_width - 40)
		{
			rectX = rectX - (40 - (m_width - FBSynthHandle::x()));
		}
		...
		QString freq = QString::number(xPixelToFreq(FBSynthHandle::x(), m_width));
		QString res;
		if (getType() != para)
		{
...
QPainterPath FBSynthHandle::getCurvePath()
{
	QPainterPath path;
...
void FBSynthHandle::loadPixmap()
{
	QString fileName = "handle" + QString::number(m_numb+1);
	if (!isActiveHandle()) { fileName = fileName + "inactive"; }
	m_circlePixmap = PLUGIN_NAME::getIconPixmap(fileName.toLatin1().constData());
}

bool FBSynthHandle::mousePressed() const
{
...
float FBSynthHandle::getPeakCurve(float x)
{
	double freqZ = xPixelToFreq(FBSynthHandle::x(), m_width);
	const int SR = Engine::audioEngine()->outputSampleRate();
	double w0 = 2 * std::numbers::pi_v<double> * freqZ / SR ;
	double c = cosf(w0);
	double s = sinf(w0);
	double Q = getResonance();
	double A =  pow(10, yPixelToGain(FBSynthHandle::y(), m_heigth, m_pixelsPerUnitHeight) / 40);
...
float FBSynthHandle::getHighShelfCurve(float x)
{
	double freqZ = xPixelToFreq(FBSynthHandle::x(), m_width);
	const int SR = Engine::audioEngine()->outputSampleRate();
	double w0 = 2 * std::numbers::pi_v<double> * freqZ / SR;
	double c = cosf(w0);
	double s = sinf(w0);
	double A =  pow(10, yPixelToGain(FBSynthHandle::y(), m_heigth, m_pixelsPerUnitHeight) * 0.025);
...
float FBSynthHandle::getLowShelfCurve(float x)
{
	double freqZ = xPixelToFreq(FBSynthHandle::x(), m_width);
	const int SR = Engine::audioEngine()->outputSampleRate();
	double w0 = 2 * std::numbers::pi_v<double> * freqZ / SR ;
...
float FBSynthHandle::getLowCutCurve(float x)
{
	double freqZ = xPixelToFreq(FBSynthHandle::x(), m_width);
	const int SR = Engine::audioEngine()->outputSampleRate();
	double w0 = 2 * std::numbers::pi_v<double> * freqZ / SR ;
	...
	double A = pow(10, yPixelToGain(FBSynthHandle::y(), m_heigth, m_pixelsPerUnitHeight) / 20);
...
float FBSynthHandle::getHighCutCurve(float x)
{
	double freqZ = xPixelToFreq(FBSynthHandle::x(), m_width);
	const int SR = Engine::audioEngine()->outputSampleRate();
	double w0 = 2 * std::numbers::pi_v<double> * freqZ / SR ;
...
float FBSynthHandle::getResonance()
{
	return m_resonance;
}
...
void FBSynthHandle::setType(int t)
{
	FBSynthHandle::m_type = t;
}




void FBSynthHandle::setResonance(float r)
{
	FBSynthHandle::m_resonance = r;
}




bool FBSynthHandle::isMouseHover()
{
	return m_mouseHover;
}
...
void FBSynthHandle::setMouseHover(bool d)
{
	m_mouseHover = d;
}
...
bool FBSynthHandle::isActiveHandle()
{
	return m_active;
}
...
void FBSynthHandle::setHandleActive(bool a)
{
	FBSynthHandle::m_active = a;
}




void FBSynthHandle::sethp12()
{
	m_hp12 = true;
	m_hp24 = false;
	m_hp48 = false;
...
double FBSynthHandle::calculateGain(const double freq, const double a1, const double a2, const double b0, const double b1, const double b2)
{
	const int SR = Engine::audioEngine()->outputSampleRate();

	const double w = 2 * std::numbers::pi_v<double> * freq / SR ;
...
void FBSynthHandle::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
...
void FBSynthHandle::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
	if (event->button() == Qt::LeftButton)
	{
		m_mousePressed = false;
...
void FBSynthHandle::wheelEvent(QGraphicsSceneWheelEvent *wevent)
{
...
void FBSynthHandle::hoverEnterEvent(QGraphicsSceneHoverEvent *hevent)
{
	setMouseHover(true);
}
...
void FBSynthHandle::hoverLeaveEvent(QGraphicsSceneHoverEvent *hevent)
{
	setMouseHover(false);
}
...
QVariant FBSynthHandle::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant &value)
{
...
// But FBSynthCurve generates an average curve.
//
// ----------------------------------------------------------------------

FBSynthCurve::FBSynthCurve(QList<FBSynthHandle*> *handle, int x, int y) :
	m_handle(handle),
	m_width(x),
	m_heigth(y),
	m_alpha(0),
...
QRectF FBSynthCurve::boundingRect() const
{
...
void FBSynthCurve::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	painter->setRenderHint(QPainter::Antialiasing, true);
	if (m_modelChanged)
	{
...
void FBSynthCurve::setModelChanged(bool mc)
{
	m_modelChanged = mc;
}
