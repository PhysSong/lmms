...
public:
	FBSynthProcessor(f_cnt_t bufLen, FBSynthInstrument *inst, float att, float dec, float sus, float rel) :
		ringBuf(bufLen),
		filter(inst),
		env(att, dec, sus, rel, 1.f / Engine::audioEngine()->outputSampleRate())
	{
	}
...
void FBSynthInstrument::playNote(NotePlayHandle* n, SampleFrame* workingBuffer)
{
	...
	for (f_cnt_t f = 0; f < frames; ++f)
	{
		...
		proc->ringBuf.write(workingBuffer + offset + f, f_cnt_t{0}, 1);
		proc->ringBuf.movePosition(f_cnt_t{1});
...
{
	return 4 * Engine::audioEngine()->outputSampleRate();
}
