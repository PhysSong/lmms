...
double * organicInstrument::s_harmonics = NULL;
...
/*	m_osc[0]->m_harmonic = log2( 0.5f );	// one octave below
	m_osc[1]->m_harmonic = log2( 0.75f );	// a fifth below
	m_osc[2]->m_harmonic = log2( 1.0f );	// base freq
	m_osc[3]->m_harmonic = log2( 2.0f );	// first overtone
	m_osc[4]->m_harmonic = log2( 3.0f );	// second overtone
	m_osc[5]->m_harmonic = log2( 4.0f );	// .
	m_osc[6]->m_harmonic = log2( 5.0f );	// .
	m_osc[7]->m_harmonic = log2( 6.0f );	// .*/
	
	if( s_harmonics == NULL )
	{
		s_harmonics = new double[ NUM_HARMONICS ];
		s_harmonics[0] = log2( 0.5f );
		s_harmonics[1] = log2( 0.75f );
		s_harmonics[2] = log2( 1.0f );
		s_harmonics[3] = log2( 2.0f );
		s_harmonics[4] = log2( 3.0f );
		s_harmonics[5] = log2( 4.0f );
		s_harmonics[6] = log2( 5.0f );
		s_harmonics[7] = log2( 6.0f );
		s_harmonics[8] = log2( 7.0f );
		s_harmonics[9] = log2( 8.0f );
		s_harmonics[10] = log2( 9.0f );
		s_harmonics[11] = log2( 10.0f );
		s_harmonics[12] = log2( 11.0f );
		s_harmonics[13] = log2( 12.0f );
		s_harmonics[14] = log2( 13.0f );
		s_harmonics[15] = log2( 14.0f );
		s_harmonics[16] = log2( 15.0f );
		s_harmonics[17] = log2( 16.0f );
	}
...
{
	m_detuningLeft = pow( 2.0, organicInstrument::s_harmonics[ static_cast<int>( m_harmModel.value() ) ]
				+ (float)m_detuneModel.value() * CENT ) /
				Engine::mixer()->processingSampleRate();
	m_detuningRight = pow( 2.0, organicInstrument::s_harmonics[ static_cast<int>( m_harmModel.value() ) ]
				- (float)m_detuneModel.value() * CENT ) /
				Engine::mixer()->processingSampleRate();
}
