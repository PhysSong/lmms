	...
	Instrument(instrumentTrack,&midioutinstr_plugin_descriptor)
{
	...
	{
		m_channelsCc[i] = make_unique<IntModel>(0, 0, MidiChannelCount, this, tr("CH%1").arg(i));
		m_controlsCc[i] = make_unique<IntModel>(0, 0, MidiControllerCount - 1, this, tr("CC%1").arg(i));
		m_valuesCc[i] = make_unique<IntModel>(0, 0, MidiMaxControllerValue, this, tr("VAL%1").arg(i));
...
PluginView* MidiOutInstr::instantiateView(QWidget* parent)
{
	return new MidiOutInstrView(this, parent);
}
