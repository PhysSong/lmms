...
public:
	...
	inline void setUserWave(std::shared_ptr<const SampleBuffer> wave)
	{
		m_userWave = wave;
	...
	{
		m_waveform = waveform;
		for (int i = 0; i < m_nvoice; ++i) m_pOsc[i]->setUserAntiAliasWaveTable(m_waveform);
	}
	void update(SampleFrame * buf, const f_cnt_t frames,
...
private:
	...
	std::shared_ptr<const OscillatorConstants::waveform_t> m_waveform;
	NuOsc * m_sub;
