...
void SampleTCOView::paintEvent( QPaintEvent * pe )
{
	...
	QColor c = getColorForDisplay(m_tco->isRecord() ? recordingBackgroundColor() : painter.background().color());
	...
	m_isPlaying(false)
{
	...
	connect (Engine::getSong(), SIGNAL(beforeRecordOn(TimePos)), this, SLOT(beforeRecordOn(TimePos)));
...
void SampleTrack::beforeRecordOn(TimePos time)
{
	if (isRecord ()) {
