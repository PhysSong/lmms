...
void NotePlayHandle::updateFrequency()
{
	...
	if (m_instrumentTrack->m_microtuner.enabled())
	{
		...
		if (m_instrumentTrack->isKeyMapped(transposedKey))
		{
			...
			m_frequency = frequency * std::exp2((detune + instrumentPitch / 100) / 12.0);
			m_unpitchedFrequency = frequency * std::exp2(detune / 12.0);
		}
		else
		{
	...
	else
	{
		...
		m_frequency = DefaultBaseFreq * std::exp2(pitch + instrumentPitch / (100 * 12.0));
